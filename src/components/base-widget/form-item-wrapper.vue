...
export default {
  ...
  computed: {
    ...
    hasFromParent() {
      let path = []
      const findPath = (pathArr, widgetList, id) => {
        widgetList &&
          widgetList.map(widget => {
            if (widget.id != id) {
              findPath([...pathArr, widget], widget.widgetList, id)
            } else {
              path = [...pathArr, widget]
            }
          })
      ...
      if (!this.parentWidget) {
        ...
        findPath([], [...this.designer.layers, ...this.designer.widgetList], this.parentWidget.id)
        path.map(item => {
          if (item.type === 'form') {
            status = true
          }
...
.el-form--inline .static-content-item {
  padding-right: 20px;
}
.static-content-item {
  min-height: 20px;
